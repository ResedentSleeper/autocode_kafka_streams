...
class KafkaTopicBuilderTest {
    ...
    private KafkaTopicBuilder kafkaTopicApplication;

    @BeforeEach
    void setup() {
        Properties properties = new Properties();
        properties.put(AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG, KAFKA_CONTAINER.getBootstrapServers());
        kafkaTopicApplication = new KafkaTopicBuilder(properties);
    }

    @Test
    void createNewTopicTest() throws Exception {
        ...
        String stdout = KAFKA_CONTAINER.execInContainer("/bin", "-c", topicCommand)
          .getStdout();
